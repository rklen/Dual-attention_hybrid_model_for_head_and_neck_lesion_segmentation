...
class Decoder(nn.Module):
    def __init__(self, dims, embed_dim, num_classes, image_size): # Added image_size
        super().__init__()
    ...
    def forward(self, inputs: Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]) -> torch.Tensor:
        ...
        c = self.dropout(c)
        out = self.linear_pred(c)
        out = F.interpolate(out, size=(self.image_size, self.image_size), mode='bilinear', align_corners=False) # Upsample to desired image_size
        return out
...
class SegFormer(nn.Module):
    def __init__(self, model_name: str = 'B0', num_classes: int = 1, image_size: int = 224) -> None:
        super().__init__()
        ...
        self.decode_head = Decoder(dims, embed_dim, num_classes, image_size) # Pass image_size to Decoder
...
seqmodel = SegFormer(image_size=224, num_classes=1)

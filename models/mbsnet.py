...
class up_conv(nn.Module):
    ...
    def forward(self, x):
        # use provided scale_factor and mode (align_corners only valid for bilinear)
...
class MBSNet(nn.Module):
    ...
    def __init__(self, in_ch=1, out_ch=1, if_adb=False):
        ...
        self.if_adb = if_adb
        self.out_ch = out_ch
        self.in_ch = in_ch  # store input channels
        ...
        # use self.in_ch here instead of hard-coded 3
        self.D_Conv4 = conv_block(filters[0] + self.in_ch, filters[0])
